<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0f0c29,50:302b63,100:24243e&height=220&section=header&text=RAVI%20KUMAR%20KESHARI&fontSize=42&fontColor=00F5D4&fontAlignY=38&animation=fadeIn&desc=Software%20Engineer%20%7C%20Distributed%20Systems%20%2B%20Generative%20AI&descAlignY=58&descSize=18&descColor=8892B0" width="100%"/>

<img src="https://readme-typing-svg.herokuapp.com?font=JetBrains+Mono&weight=600&size=22&duration=3000&pause=1000&color=00F5D4&center=true&vCenter=true&width=800&lines=Architecting+production-grade+platforms+for+100K%2B+users;NestJS+%2B+Next.js+%2B+PostgreSQL+%2B+Redis;Generative+AI+%7C+Multimodal+RAG+%7C+Diffusion+Models;IIT+Patna+%E2%80%94+CS+%26+Data+Analytics+%2724-%2728" />

<br/>

[![Portfolio](https://img.shields.io/badge/PORTFOLIO-0A0A0A?style=for-the-badge&logo=vercel&logoColor=00F5D4)](https://www.ravikeshari.me/)
[![LinkedIn](https://img.shields.io/badge/LINKEDIN-0A0A0A?style=for-the-badge&logo=linkedin&logoColor=0A66C2)](https://linkedin.com/in/ravi-kumar-keshari-a37828347)
[![LeetCode](https://img.shields.io/badge/LEETCODE-0A0A0A?style=for-the-badge&logo=leetcode&logoColor=FFA116)](https://leetcode.com/u/ravi_c029/)
[![X](https://img.shields.io/badge/X-0A0A0A?style=for-the-badge&logo=x&logoColor=white)](https://x.com/ravi_c029)
[![Mail](https://img.shields.io/badge/MAIL-0A0A0A?style=for-the-badge&logo=gmail&logoColor=EA4335)](mailto:ravi.keshari029@gmail.com)

<br/>

<img src="https://komarev.com/ghpvc/?username=ravi-c029&label=PROFILE+VIEWS&color=00F5D4&style=for-the-badge" alt="Profile Views"/>
<img src="https://img.shields.io/github/followers/ravi-c029?style=for-the-badge&color=0A0A0A&labelColor=0A0A0A&logo=github&logoColor=00F5D4"/>

</div>

<br/>

## 👨‍💻 Initialization

<br/>

<table>
<tr>
<td width="58%" valign="top">

```yaml
name:        Ravi Kumar Keshari
education:
  institute: Indian Institute of Technology Patna
  program:   B.S. Computer Science & Data Analytics
  timeline:  2024 – 2028

core_expertise:
  - Scalable Backend Infrastructure
  - Generative AI & Diffusion Models
  - Retrieval-Augmented Generation (RAG)
  - Full Stack Product Engineering
  - System Design for High Concurrency

philosophy: >
  Ship systems that scale, measure everything,
  and refactor without mercy.
```

</td>
<td width="42%" align="center">
<img src="https://raw.githubusercontent.com/ravi-c029/ravi-c029/main/assets/hero-coding.svg" width="100%"/>
</td>
</tr>
</table>

<br/>

## 🧠 Core Expertise

<br/>

| Domain | Proficiency | Details |
|---|---|---|
| **Full Stack Engineering** | ██████████ | Next.js, NestJS, Flutter — end-to-end multi-surface architecture |
| **Backend & System Design**| ██████████ | Distributed systems, Redis queues, REST/WebSockets, high concurrency |
| **Generative AI & RAG**    | █████████░ | Multimodal RAG pipelines, Diffusion Models, ViT, `pgvector` |
| **Data Science**           | █████████░ | Pandas, NumPy, scikit-learn — analysis & applied ML |
| **DevOps & Cloud**         | ████████░░ | Docker, CI/CD pipelines, AWS, Edge deployment |
| **DSA & Algorithms**       | █████████░ | Daily problem solving, optimized system design for hackathons |

<br/>

## ⚡ Technology Stack

<br/>

<div align="center">

**Core Languages**
<br/>
<img src="https://skillicons.dev/icons?i=ts,js,py,cpp,dart,html,css&theme=dark"/>
<br/><br/>

**Backend & Databases**
<br/>
<img src="https://skillicons.dev/icons?i=nestjs,nodejs,express,graphql,prisma,postgres,redis,mongodb,supabase&theme=dark"/>
<br/><br/>

**AI & Data Engineering**
<br/>
<img src="https://skillicons.dev/icons?i=pytorch,tensorflow,opencv,r&theme=dark"/>
<br/><br/>

**Frontend & Mobile**
<br/>
<img src="https://skillicons.dev/icons?i=nextjs,react,flutter,tailwind,vercel&theme=dark"/>
<br/><br/>

**Cloud, DevOps & Tooling**
<br/>
<img src="https://skillicons.dev/icons?i=aws,docker,cloudflare,githubactions,linux,git,github,vscode&theme=dark"/>

</div>

<br/>

## 🛰️ Featured Builds

<br/>

<table width="100%">
<tr>
<td width="50%" valign="top">

### 🩺 DocMateX
**Healthcare Networking Ecosystem**
Architected an MVP for **100,000+ users**. Engineered a decoupled backend using Redis queues for asynchronous job processing, ensuring non-blocking real-time mentor matching and notifications.
<br/>
`Next.js 15` `Flutter` `NestJS` `Prisma` `PostgreSQL` `Redis` `Socket.IO` `BullMQ`

</td>
<td width="50%" valign="top">

### 📚 Leazy Learn
**AI-Powered EdTech SaaS**
Built a high-throughput **multimodal RAG tutor** trained on NCERT data. Parsed, chunked, and embedded textbooks using `pgvector`, grounding answers in both text and visual diagrams. 119+ production API routes.
<br/>
`Next.js` `Supabase` `pgvector` `LiveKit` `Groq / OpenAI` `Razorpay`

</td>
</tr>
<tr>
<td width="100%" colspan="2" valign="top">

### 🎙️ Interview Twin
**AI Mock Interview Simulator**
Designed a full system architecture with a real-time low-latency voice pipeline and adaptive technical question generation for interview simulation.
<br/>
`Next.js` `Supabase` `Cartesia AI` `WebRTC`

</td>
</tr>
</table>

<br/>

## 🏆 Achievements

<br/>

| Recognition | Details |
|---|---|
| 🥉 **HackNtech 3.0** | Secured **3rd Position** overall in the hackathon, competing against top teams. |
| 📚 **Leazy Learn** | Successfully shipped a production AI EdTech SaaS with a multimodal RAG tutor serving real students. |

<br/>

## 🎯 Current Focus

<br/>

```yaml
learning:
  - Advanced Distributed Systems (Paxos/Raft architectures)
  - Edge AI & On-Device Inference optimization
  - Advanced Data Structures & Algorithms

building:
  - DocMateX — Enterprise-grade healthcare ecosystem
  - Leazy Learn — Agentic AI Tutor workflows

exploring:
  - AI Safety, Alignment, and Agentic Workflows
  - Multi-Sensor Satellite Data Fusion

open_to:
  - SDE Internships (Backend / Fullstack)
  - Research Collaborations in Applied AI
  - Open Source Contributions
  - Hackathons & System Design Competitions
```

<br/>

## 📊 Live System Metrics

<br/>

<table width="100%" align="center">
  <tr>
    <td width="50%" align="center">
      <img src="https://github-readme-stats.vercel.app/api?username=ravi-c029&show_icons=true&theme=chartreuse-dark&hide_border=true&bg_color=0A0A0A&title_color=00F5D4&icon_color=00F5D4&text_color=8892B0&count_private=true&cache_seconds=86400" alt="GitHub Stats" />
    </td>
    <td width="50%" align="center">
      <img src="https://github-readme-streak-stats.herokuapp.com/?user=ravi-c029&theme=nightowl&hide_border=true&background=0A0A0A&ring=00F5D4&fire=00F5D4&currStreakLabel=00F5D4&cache_seconds=86400" alt="Streak Stats" />
    </td>
  </tr>
  <tr>
    <td width="50%" align="center">
      <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=ravi-c029&layout=compact&theme=chartreuse-dark&hide_border=true&bg_color=0A0A0A&title_color=00F5D4&text_color=8892B0&cache_seconds=86400" alt="Top Languages" />
    </td>
    <td width="50%" align="center">
      <img src="https://github-profile-trophy.vercel.app/?username=ravi-c029&theme=darkhub&no-frame=true&no-bg=true&margin-w=8&row=1&column=4&cache_seconds=86400" alt="Trophies" />
    </td>
  </tr>
</table>

<br/>

<div align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=ravi-c029&theme=react-dark&hide_border=true&bg_color=0A0A0A&color=00F5D4&line=00F5D4&point=F75C7E" width="100%" alt="Activity Graph"/>
</div>

<br/>

<!-- Animated, real-time contribution snake. Generated by .github/workflows/snake.yml -->
<div align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/ravi-c029/ravi-c029/output/github-contribution-grid-snake-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/ravi-c029/ravi-c029/output/github-contribution-grid-snake.svg">
    <img alt="GitHub Contribution Snake" src="https://raw.githubusercontent.com/ravi-c029/ravi-c029/output/github-contribution-grid-snake-dark.svg" width="100%">
  </picture>
</div>

<br/>

<div align="center">
  <img src="https://leetcard.jacoblin.cool/ravi_c029?theme=dark&font=JetBrains%20Mono&ext=heatmap" width="100%" alt="Leetcode Stats"/>
</div>

<br/>

<div align="center">

<samp>"Scalability is not an afterthought; it is an architectural prerequisite."</samp>

<br/><br/>

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:24243e,50:302b63,100:0f0c29&height=100&section=footer" width="100%"/>

</div>
